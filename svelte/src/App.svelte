<script>
  ...
  async function handleConnect() {
    ...
    if (!key) {
      error = { message: 'Please enter a Workspace API Key', hint: 'Get it from Settings → Workspace in the dashboard.' };
      return;
    }
    if (UUID_REGEX.test(key)) {
      error = {
        message: 'That looks like a Workspace ID, not an API key',
        hint: 'Use the Workspace API Key from Settings → Workspace (create/copy key). Do not use the workspace UUID.'
      };
      return;
    }
    if (!clientId?.trim()) {
  ...
  function finalizeStreamingMessage() {
    // Find and finalize any in-progress streaming message so it's preserved
    const idx = messages.findLastIndex(m => m.role === 'assistant' && m.streaming);
    if (idx !== -1) {
      messages[idx] = { role: 'assistant', text: messages[idx].text };
      messages = [...messages];
    }
  }
  ...
  function handleAssistantMessage(e) {
    ...
    if (done) {
      // Replace the current streaming message with the final complete one
      const idx = messages.findLastIndex(m => m.role === 'assistant' && m.streaming);
      if (idx !== -1) {
        messages[idx] = { role: 'assistant', text };
        messages = [...messages];
      } else {
      ...
      if (last?.role === 'assistant' && last.streaming) {
        ...
        // New streaming response starting — finalize any previous one first
        finalizeStreamingMessage();
...
<div class="container">
  ...
  <div class="form-group">
    <label for="workspaceKey">Workspace API Key</label>
    <input
      type="password"
      id="workspaceKey"
      bind:value={workspaceKey}
      placeholder="Paste API key (not the Workspace ID)"
      disabled={status !== 'disconnected'}
    />
    <p class="hint">Settings → Workspace → create/copy API key. Use the key, not the workspace UUID.</p>
  </div>
