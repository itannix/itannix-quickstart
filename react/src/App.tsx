...
function App() {
  ...
  const finalizeStreamingMessage = () => {
    setMessages(prev => {
      ...
      const next = [...prev];
      next[idx] = { role: 'assistant', text: next[idx].text };
      return next;
    });
  };
      ...
      if (done) {
        ...
        setMessages(prev => {
          ...
          const next = prev.map(m =>
            m.role === 'assistant' && m.streaming ? { role: 'assistant' as const, text: m.text } : m
          );
          return [...next, { role: 'assistant' as const, text, streaming: true }];
        });
      }
    },
    onError: (err) => {
  ...
  const handleConnect = async () => {
    if (!workspaceKey) {
      setError({ message: 'Please enter a Workspace Key' });
      return;
    }
    if (!clientId) {
    ...
    <>
      ...
      <div className="container">
        ...
        <div className="form-group">
          ...
          />
          <p className="hint">From Settings → Workspace in the dashboard</p>
        </div>
